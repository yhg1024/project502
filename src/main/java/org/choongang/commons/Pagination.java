...
public class Pagination {
     ...
     *
     * @param page : 현재 페이지
     * @param total : 전체 레코드 갯수(총 페이지가 몇개인지?)
     * @param ranges : 페이지 구간 갯수 (1이상)
     * @param limit : 한페이지 당 레코드 갯수 (양수)
     */
    public Pagination(int page, int total, int ranges, int limit, HttpServletRequest request) {
        // request에서 쿼리스트링 가져가 쓰기
        ...
        // 전체 페이지 갯수
        // 인트를 나누면 소수점 버림이라 한개를 double로 만들고 다시 인트로 0
        ...
        */
        ...
        if (request != null) {
            ...
            if (StringUtils.hasText(queryString)) {
                ...
                baseURL += Arrays.stream(queryString.split("&")) // &로 쪼갠다.
                        .filter(s -> !s.contains("page=")) // page가 포함된거는 제외한다.
    ...
    public List<String[]> getPages() {
        ...
        return IntStream.rangeClosed(firstRangePage, lastRangePage)
                ...
                .toList();
        // String.valueOf(p) : 페이지 번호
        // "?page=" + p : 페이지 링크(2를 누르면 2페이지로 가는거)
        // 반환값 내보내기 위한 용도로 변수를 쓰니까 그대로 넣을거면 변수 안쓰고 반환해도 된다.
    }
}
